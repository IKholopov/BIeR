...
CallOp::CallOp(const Function* context, const Function* function, std::optional<const Value*> return_value,
    ...
    check( (!return_value_.has_value() && !function->GetSignature()->ReturnType().has_value())
           ...
           std::runtime_error("call to function " + function->GetName() + " does not match return type"));
    int index = 0;
    check(function->GetSignature()->Arguments().Size() == arguments.size(), std::runtime_error("not enough arguments"));
...
void CallOp::CheckReturnAndArgs() const
{
    check( (!return_value_.has_value() && !type_->ReturnType().has_value())
           || (return_value_.has_value() && return_value_.value()->GetType() == type_->ReturnType().value()),
