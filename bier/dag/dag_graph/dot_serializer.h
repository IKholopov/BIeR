...
class DagDotSerializer {
public:
    explicit DagDotSerializer(std::ostream& dag_stream);
    ~DagDotSerializer();

    void Serialize(const VisualOpDag* graph, const std::string& label);

private:
    std::ostream& stream_;
    int subgraphs_counter_ = 0;
